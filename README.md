<h1 align="center">Vishal Vishwakarma</h1>
<h3 align="center">Web3 Engineer (Solana) • Full Stack • Real-Time Systems</h3>

## ⚡ About Me

- Building scalable full-stack applications
- Engineering real-time systems (WebRTC, event-driven flows)
- Developing Web3 applications on Solana
- Exploring AI-driven systems & intelligent automation
- Focused on performance, security & clean architecture

---

## 📊 Activity

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=vshl01&theme=tokyo-night&hide_border=true"/>
</p>

---

## 🧠 Tech Stack

### Frontend

`React` `Next.js` `TypeScript` `React Native`

### Backend

`Node.js` `Express` `Firebase` `REST APIs`

### Real-Time & Systems

`WebRTC` `Socket-based architectures` `Event-driven systems`

### Web3

`Solana` `Smart Contracts` `Wallet Integrations` `On-chain logic` `Solana Mobile`

### Cloud & Infra

`Google Cloud` `Firebase` `CI/CD` `GitHub Actions` `Kubernetes`

---

## 🚀 Featured Work

- 🔐 **D-Cash** – Encrypted digital transaction platform
- 🖥 **WebRTC Screen Share System** – Real-time remote control architecture
- 🌐 **Solana Web3 Projects** – On-chain interaction & wallet integration

---

<p align="center">
  <img src="https://github.com/vshl01/vshl01/blob/output/github-contribution-grid-snake.svg" />
</p>

---

## 🌍 Connect

- Web3 / Solana Builder
- Open to high-impact engineering roles
